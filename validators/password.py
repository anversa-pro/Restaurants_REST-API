...
class Password:
    ...
    def validate_password(arg):
        ...
        if len(arg) < 10:
            raise PasswordLengthError("the password must have at least 10 characters.")
        else:
    ...
    def validate_requirements(arg):
        ...
        if not upper:
            raise PasswordUpperCaseError("the password must have at least one uppercase letter.")

        lower = (re.search(r"[a-z]", arg))
        if not lower:
            raise PasswordLowerCaseError("the password must have at least one lowercase letter.")
